...
<?php
//Test des varriable et redirection vers les differentes étapes d'ajout d'actionneur en fonction des varriables nulles ou non
...
if($_SESSION['finalisation']=='Valider')
{
  ...
  $req->execute(array('id_capteur_actionneur'=>$_SESSION['actionneur'], 'nom_piece'=>$_SESSION['piece'], 'code'=>$_SESSION['code']));
  header('Location: ../vues/vue_tableau_de_bord.php?titre=Tableau de bord');
}
elseif($_SESSION['finalisation']=='Annuler')
{
  unset($_SESSION['actionneur'], $_SESSION['piece']);
  header('Location: ../vues/vue_tableau_de_bord.php?titre=Tableau de bord');
}
